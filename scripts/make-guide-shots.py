...
SHOTS = {
    ...
    "exp-old": [
        ...
        seg(f"{EXP}/05_基護/05_基護_詳解_Ch01_緒論.pdf", 7, "針對護理學者", "107-2-基護行政-3"),
    ],
    "exp-disputed": [seg(f"{EXP}/05_基護/05_基護_詳解_Ch05_感染控制.pdf", 59,
                         "有關輪狀病毒", "106-2-產兒-76")],
    # 表 11-1 跨兩頁：第 6 頁從表頭到第三列，第 7 頁從第四列接到相似考點（跳過重複的表頭）。
    # 連結加了底線後文字會被拆段，「106-2-內外-18」搜不到整串，改用清單裡第一個「-18」。
    "exp-table": [seg(f"{EXP}/05_基護/05_基護_詳解_Ch11_冷熱療法.pdf", 5,
                      "表11-1", "冷降低代謝"),
                  seg(f"{EXP}/05_基護/05_基護_詳解_Ch11_冷熱療法.pdf", 6,
                      "血液黏稠度", "-18", highlight=True)],
    "exp-table-ref": [seg(f"{EXP}/10_精神/10_精神_詳解_Ch08_雙相情緒障礙症病人的護理.pdf", 28,
    ...
    "wb-answers": [seg(f"{WB}/11_社區/11_社區_題本_Ch02_流行病學.pdf", 20,
                       "參考答案", "86.", highlight=True)],
    "wb-origin": [seg(f"{WB}/11_社區/11_社區_題本_Ch02_流行病學.pdf", 17,
                      "跨章考題", "114-3-精社-30", pad_top=2)],
    # ---------- 歷年考題 ----------
    "pe-page": [seg(f"{PE}/115-1/115-1_產兒科護理學.pdf", 2,
                    "產兒科護理學", "安撫奶嘴會改變")],
}
